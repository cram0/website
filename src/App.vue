        ...
        <div id="center-box" :class="['flex', 'justify-center', 'items-center', 'transition-all', setMainMenuSizeBasedOnPage(), 'pb-2']">
            <div :class="[mainMenuExpanded ? expandedStyle : notExpandedStyle]">
                ...
                <ul v-if="isMainMenu" v-for="choice in choiceList" class="breathing-select-red text-center" @mouseenter="playSound('hover')" @mousedown="playSound('click')">
                    <router-link v-if="!choice.externalLink" :to="{ path: choice.path }" @click="isMainMenu = !isMainMenu" class="block">{{ t(choice.name.toLowerCase()) + " " }}<img v-if="choice.icon" :src="'/' + choice.icon" class="h-4 inline-block" /></router-link>
